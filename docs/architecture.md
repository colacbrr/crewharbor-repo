...
query
  ...
  -> Ollama explanation
  -> structured JSON + fallback formatting
```
...
The top retrieved results are passed into a local Ollama-based explanation workflow.
...
The prompt asks for strict JSON with a summary, uncertainty value, and per-item references.
...
The frontend is the interactive layer for the retrieval system.
...
This keeps the system easy to inspect and operate during development and evaluation.
